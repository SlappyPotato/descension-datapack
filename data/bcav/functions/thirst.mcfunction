...
#water drinking system--------------------------------------------------------
	...
	execute as @a store success score @s holdAnt if entity @s[nbt={SelectedItem:{id:"minecraft:potion",tag:{Antidote:1b,Purified:2b}}}]
	...
	execute as @a[scores={drankWater=1..,ant_tim=1..}] at @s run scoreboard players remove @s Toxicity 30
