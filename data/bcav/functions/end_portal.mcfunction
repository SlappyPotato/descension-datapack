#making end portal
execute as @e[type=minecraft:item,nbt={Item:{id:"minecraft:nether_star",Count:1b}}] at @s if block ~ ~-1 ~ minecraft:bedrock if entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:ender_eye",Count:1b}},distance=..1] unless entity @e[type=minecraft:item,nbt={Item:{id:"minecraft:blaze_powder",Count:1b}},distance=..2] unless score Target EPORT_BUILT matches 1 run tag @s add eport_item
